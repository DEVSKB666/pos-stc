...
<div class="col px-3">
    <div class="container rounded-3 shadow-sm bg-white mt-3">
        <h4 class="fw-bold text-center pt-3">ออเดอร์ของฉัน</h4>
            <?php 
                if($num <= 0) {
                    ...
                    <div class="card-body">
                        ...
                        <p>ราคารวม <?= $fet->total_price; ?> บาท</p>
                                <p>สถานะ : <?= order_status($fet->order_status); ?> </p>
                                        ...
                                        ?>
                                <button id="btn-complete" data-id="<?= $fet->order_id; ?>" class="btn btn-outline-success btn-sm">ยืนยันการส่งอาหารเและชำระเงินเสร็จสิ้น</button>
                        <?php } ?>
                        </div>
                    </div>
                </div>
